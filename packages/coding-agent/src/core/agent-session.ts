...
export class AgentSession {
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		if (
			...
			return true;
		// Bug 2b: narrow transient-text fallback. Some providers surface a
		// clearly-transient error whose body carries none of the keyword/structured
		// signals above. The canonical case is an unmodeled AWS Bedrock mid-stream
		// exception, thrown by Smithy as a bare `Error(body)` with no
		// BedrockRuntimeServiceException type/prefix, e.g.
		//   {"message":"The system encountered an unexpected error during processing. Try your request again."}
		// (modeled exceptions already get a category prefix that the regex above catches;
		// this only adds the residual bare/unknown-code case). The billing/quota veto runs
		// BEFORE this, so transient phrasing never overrides a genuine usage-limit error.
